...
namespace Lame\TypeScript\Generators;

use Lame\TypeScript\Definitions\TypeScriptProperty;
use Lame\TypeScript\Definitions\TypeScriptType;
...
class RequestGenerator extends AbstractGenerator
{
    ...
    private function parseRules(string $property, array|string $rules): ?array
    {
        ...
        if ($rules->has('confirmed')) {
            $extraProperty = $rules['confirmed'] ?? "{$property}_confirmation";
    ...
    private function resolveColumn(string $property, ?string $args): ?string
    {
        ...
        $table = $prefix . $table;
        if (!Schema::hasTable($table) || !Schema::hasColumn($table, $columnName)) {
            return null;
        }
